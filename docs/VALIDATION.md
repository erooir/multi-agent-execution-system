...
- Backend: 42 tests passed. Includes budget reservation concurrency, uncertain charges, endpoint/model allowlist, local-only evidence blocking, role enforcement, workflow validation/versioning, real Agno execution over mocked model HTTP, cancellation, report rejection/rework, output truncation detection and report exports.
- Python: Ruff lint and formatting checks passed (19 files).
- Frontend: TypeScript and Vite production build passed; four DAG layout tests passed. Accessible form labels were checked against rendered markup.
...
At acceptance, ten metered requests had a conservative total of CNY 0.28464 and no unresolved reservations. This ledger estimate is based on twice the verified peak token rates, not the provider invoice. The persistent project ceiling is CNY 300. Tests use mocks and make no paid calls.
